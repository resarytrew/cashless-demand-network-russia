"""An offline, descriptive reader for the authenticated Atlas CSV."""
import json
...
def build_report(output, reference_path, settings):
    frame = pd.read_csv(output / "municipality_affinity_atlas.csv")
    reference = pd.read_csv(reference_path, index_col=0)
    keep = ["municipality", "reference_profile", "consensus_class", "second_class",
            "affinity_margin", "affinity_entropy", "lofo_class_match_count",
            "anchor_consensus_class", "D_G_transition_direction",
            "perturbation_peer_coassignment", "destination_precision", "destination_jaccard",
            "stability_class"] + [f"affinity_share_{p}" for p in "ABCDEFG"]
    selected = frame[keep].copy()
    selected["trajectory"] = [reference.loc[name].astype(int).tolist()
                               for name in selected.municipality]
    payload = selected.to_json(orient="records", force_ascii=False).replace("<", "\\u003c")
    template = Path(__file__).with_name("atlas_report.html").read_text(encoding="utf-8")
    output.joinpath("index.html").write_text(template.replace("__ATLAS_DATA__", payload)
        .replace("__MONTHS__", json.dumps(reference.columns.tolist()))
        .replace("__ATLAS_VERSION__", settings["atlas_version"])
        .replace("__REFERENCE_MONTH__", settings["reference_month"][:7])
        .replace("__ANCHOR_PERCENT__", str(100 * settings["anchor_fraction"])), encoding="utf-8")
    output.joinpath("DATA_DICTIONARY.md").write_text('''# Atlas 2.2.1 data dictionary
